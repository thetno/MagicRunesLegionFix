...
function mod:UpdateBarIcons()
   for id,data in pairs(db.bars) do
	 ...
	 runebars[id]:SetIcon(mod:GetRuneIcon(mod:GetSpecInfo()))
      end
   end
   if mod.modules.RuneBars then
      for id, f in pairs(mod.modules.RuneBars.bars) do
	 f.icon:SetTexture(mod:GetRuneIcon(mod:GetSpecInfo()))
      end
   end
end
...
do
		...
		for id,barData in ipairs(db.bars) do
			...
			if bar then
				if barData.type == mod.RUNIC_BAR then
					if bar.value ~= currentRunicPower or bar.maxValue ~= currentMaxRunicPower then
   ...
   function mod:RUNE_TYPE_UPDATE(_, rune)
      runeData[rune].type = mod:GetSpecInfo()
      if not scriptActive then
	 mod:UpdateBars()
      end
   end
